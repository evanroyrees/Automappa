...
def find_annotations(dirpath: str) -> List[str]:
    annotation_globnames = [
        "*.coverages.tsv",
        "*.gc_content.tsv",
        "*.taxonomy.tsv",
        "*_bacteria_cache/superkingdom/superkingdom.am_clr_pca50_umap2.tsv.gz",
    ]
    annotations = []
...
def main():
    ...
    parser.add_argument(
        "--autometa-results",
        help="Path to metagenome autometa results directory",
        required=True,
    )
    parser.add_argument(
        "--out", help="Path to write merged annotations table", required=True
    )
